...
with st.sidebar:
  ...
  modelIndex=0
  if os.getenv('SELECTED_MODEL') == 'Kaito':
    modelIndex=1
  # Model selection
  selected_model = st.selectbox('Model source', ['Azure OpenAI', 'Kaito'], key='selected_model', index=modelIndex, placeholder="Choose an option", help='The source of the model to use for the chatbot.')
    
  # Display warnings for unavailable models
  if selected_model in ['Kaito']:
